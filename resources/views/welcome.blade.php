    ...
    <section class="bg-white dark:bg-zinc-900">
        <div class="py-8 px-4 mx-auto max-w-screen-xl text-center lg:py-16 lg:px-12">
            <form
                action="/search"
                method="GET"
                class="inline-flex justify-between items-center px-2 py-1 mb-7 text-sm text-gray-700 bg-gray-100 rounded-full dark:bg-gray-800 dark:text-white hover:bg-gray-200 dark:hover:bg-gray-700">

                <input
                    type="text"
                    name="query"
                    value="{{ request('query') }}"
                    class="text-sm font-medium bg-gray-100 rounded-full dark:bg-gray-800 rounded-full text-black dark:text-white border-none"
                    placeholder="Get jiggy with it!"
                    required>

                <button type="submit" class="text-xs bg-blue-600 rounded-full text-white px-4 py-2 m-0">Search</button>
            </form>
        ...
        <div class="grid grid-row-2">
            ...
            <div class="row border-t-2 border-zinc-800 p-5 flex justify-between">
                ...
                <div class="grid grid-row-1 grid-flow-col gap-4">
                    <!-- model filter -->
                    <div class="div">
                        <x-dropdown align="left" width="48" data-filter-type="make">
                            <x-slot name="trigger">
                                <button class="inline-flex items-center px-4 py-2 border border-transparent text-sm font-medium rounded-full text-zinc-500 dark:text-zinc-400 bg-white dark:bg-zinc-800 hover:text-zinc-700 dark:hover:text-zinc-300 focus:outline-none transition">
                                    <span>Make</span>
                                    <svg class="ml-2 h-4 w-4" xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 20 20" stroke="currentColor">
                            ...
                            <x-slot name="content">
                                ...
                                <x-dropdown-link data-filter-value="{{ $make->name }}">
                                    {{ $make->name }}
                                </x-dropdown-link>
                                @endforeach
                            </x-slot>
            ...
            <!-- show filters -->
            <div class="flex justify-between bg-zinc-200 dark:bg-zinc-800">
                <div class="flex flex-row gap-4 p-5">
                    <span class="inline-flex items-center px-4 py-2 text-sm font-medium rounded-full text-zinc-500 dark:text-zinc-400 bg-white dark:bg-zinc-800">
                        Filters |
                    </span>
                ...
                <div class="flex justify-center items-center p-5">
                    <button id="apply-filters"
                        class="inline-flex items-center px-4 py-2 border border-transparent font-medium 
                       rounded-full text-zinc-500 dark:text-zinc-400 bg-white dark:bg-zinc-800 
                       hover:text-zinc-700 dark:hover:text-zinc-300 focus:outline-none transition">
                        Search
                    </button>
                </div>
            </div>
